...
export function setupPortProxy(app: Express): void {
  ...
  memoryApiPaths.forEach(apiPath => {
    app.use(apiPath, (req: Request, res: Response) => {
      const target = 'http://127.0.0.1:37777';
      ...
      proxy.web(req, res, { target }, (err) => {
        ...
        res.status(502).json({
          ...
          message: 'Could not connect to claude-mem at 127.0.0.1:37777. Is it running?'
        });
      });
    });
  });

  // Memory proxy fallback - 127.0.0.1:37777 (for any other /memory/* paths)
    ...
    proxy.web(req, res, { target, timeout: 30000 }, (err: any) => {
      ...
      if (!res.headersSent) {
        res.status(502).json({
          ...
          message: `Could not connect to 127.0.0.1:${port}. Error: ${errorMsg}`
        });
      }
    });
  });
      ...
      proxy.web(req, res, { target }, (err) => {
        ...
        if (!res.headersSent) {
          res.status(502).json({
            ...
            message: `Could not connect to 127.0.0.1:${port}`
          });
        }
      });
    });
...
export function setupWebSocketProxy(server: import('http').Server): void {
  server.on('upgrade', (req, socket, head) => {
    ...
    // Handle Vite HMR WebSocket in development
    if (process.env.NODE_ENV !== 'production') {
      const protocol = req.headers['sec-websocket-protocol'];
      if (protocol && protocol.toString().includes('vite-hmr')) {
    ...
    if (isDevServerPath && activePreviewPort) {
      const target = `http://127.0.0.1:${activePreviewPort}`;
